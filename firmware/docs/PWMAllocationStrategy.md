...
* **D1:** Elevator (PWM0)  
* **D3:** Rudder (PWM0)  
* **D7:** Flaps (PWM0) — *Note: On Plus, D7 is available as GPIO however it will be used for UART2*  
...
hw.ServoPWM.Configure(machine.PWMConfig{
    Period: machine.GHz * 1 / SERVO_PWM_FREQUENCY})

// 400Hz for standard ESCs (for smoother throttle response)
hw.ESCPWM.Configure(machine.PWMConfig{
    Period: machine.GHz * 1 / ESC_PWM_FREQUENCY})   
```
## **5. Hardware Guardrails**

* **Avoid D6/D7 (Standard):** These are UART pins used for the RC Receiver (CRSF/ELRS/IBUS). On the Plus, check your specific pin map if you use GPS which will use the secondary UART.  
